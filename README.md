...
Default (no `--book`): downloads **every** title in `BOOKS` inside `src/fetch_corpus.py`, strips Project Gutenberg boilerplate, concatenates with `=== book_key ===` markers between books, writes `data/corpus.txt`, then writes truncations at **1, 5, 10, and 50 MiB** (`data/corpus_1MB.txt` through `data/corpus_50MB.txt`). Use `--sizes` to override (comma-separated byte counts).
...
`--book` choices: `moby_dick`, `alice`, `frankenstein`, `tale_of_two_cities`, `pride_and_prejudice`, `complete_works_of_shakespeare`, `war_and_peace`, `les_miserables`.
...
This is generated by the bash script, which compares GPU vs tiktoken performance at different input sizes to help see how the two implementations scale compared to each other.

```bash
scripts/run_scaling_benchmark.sh
```

Runs the combined fetch if `data/corpus.txt` is missing, then sweeps at **1, 5, 10, 50 MiB** and **full** text (`tags` `corpus_1MB` ... `corpus_full`), appends `data/bpe_benchmark_scaling.csv`, verifies each size, and runs the visualizer to write `bpe_scaling_report.txt` plus scaling PNGs.

If the CSV contains **more than one** distinct `tag` value (for example `data/bpe_benchmark_scaling.csv`), the tool writes **`bpe_scaling_report.txt`** plus **`scaling_throughput_vs_size.png`** and **`scaling_speedup_vs_size.png`**: one tiktoken baseline block per tag, a table of **every** GPU row with ratio columns `x_k_pp`, `x_e_enc`, `x_p_enc`, and curves of best-throughput vs input size (MiB) across tags.

Single-tag mode writes `throughput_vs_threads.png`, `speedup_vs_tiktoken.png`, and **`bpe_report.txt`** (short best lines per kernel plus an all-rows table with the same columns).
...
Outputs (multi-tag scaling CSV):
- `bpe_scaling_report.txt` - per-tag tiktoken baselines and full GPU tables.
- `scaling_throughput_vs_size.png` - best kernel and pipeline MB/s per tag vs input MiB, with tiktoken encode and per-piece references.
- `scaling_speedup_vs_size.png` - MB/s ratios vs `encode()` and vs per-piece loop across corpus sizes.

### Understanding the Report

The columns in the bpe_scaling_report are as follows:
